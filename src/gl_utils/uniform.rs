...
impl UniformFloatVector {
  pub fn new(name: &str, count: i8, values: Vec<f32>) -> UniformFloatVector {
    UniformFloatVector { name: String::from(name), count, values, data_type: DataType::Float32 }
  }
}
...
impl SettableUniform<Vec<f32>> for UniformFloatVector {
  ...
  fn set_uniform(&self, loc: i32) {
    match self.count {
      ...
      _ => { panic!("Uniform for vector of {} values not implemented", self.count) }
    }
  }
}
...
impl UniformInteger {
  pub fn new(name: &str, value: i32) -> UniformInteger {
    UniformInteger { name: String::from(name), value, data_type: DataType::Int }
  }
}
...
impl UniformMatrixFloat {
  pub fn new(name: &str, value: Matrix<f32>) -> UniformMatrixFloat {
    UniformMatrixFloat { name: String::from(name), value, data_type: DataType::Float32 }
  }
}
