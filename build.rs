...
fn main() {
      ...
      for file in fs::read_dir("./images/").unwrap().into_iter() {
        ...
        fs::copy(format!("./images/{}", entry_name), format!("{}/{}", out_images, entry_name)).unwrap(); 
      }
      ...
      for shader_dir in fs::read_dir("./shaders/").unwrap().into_iter() {
        ...
        for file in fs::read_dir(format!("./shaders/{}", entry_name)).unwrap().into_iter() {
          let file_name = file.unwrap().file_name().into_string().unwrap();
          fs::copy(format!("./shaders/{}/{}", entry_name, file_name), format!("{}{}/{}", out_shaders, entry_name, file_name)).expect(""); 
        }
      }
}
